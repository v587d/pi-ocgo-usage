...
const extension = (pi: ExtensionAPI): void => {
  const cache = new UsageCache(
    STATUS_KEY,
    (registry) => fetchUsage(registry),
    (data, theme) => renderUsage(data, theme) ?? "",
    (error, theme) => renderError(error, theme),
  )

  pi.on("session_start", (_event, ctx) => {
    if (isOpencodeGoProvider(ctx)) cache.refresh(ctx)
  })

  pi.on("model_select", (event, ctx) => {
    if (isOpencodeGoModel(event.model)) cache.refresh(ctx)
    else cache.clear(ctx)
  })
