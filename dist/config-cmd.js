...
function writeConfig(cookie, workspaceID) {
 ...
 */
export async function runOcgoConfig(args, ctx) {
    ...
    if (sub === "set") {
        ...
        const cookie = await ctx.ui.input("OpenCode Go session cookie (Cookie header value, e.g. auth=Fe26.2*…; oc_locale=zh)", loadConfig().cookie ?? "");
        if (!cookie) {
            ctx.ui.notify("Cancelled: cookie is required.", "warning");
            return {};
        }
        const confirmed = await ctx.ui.confirm("Persist cookie to disk?", `The cookie is a full OpenCode user session (1-year TTL). It will be written to ${configFilePath()} with mode 0600. Continue?`);
        if (!confirmed) {
    ...
    if (sub === "clear") {
        ...
        const ok = await ctx.ui.confirm("Delete config file?", `This will remove ${path}. You will need to reconfigure.`);
        if (!ok)
            return {};
    ...
    if (sub === "test") {
        ...
        try {
            ...
            const rendered = renderUsage(data, ctx.ui.theme);
            if (rendered) {
