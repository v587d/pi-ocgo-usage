...
export class UsageCache {
  ...
  constructor(
    ...
    private readonly renderStatus: (data: NormalizedUsage, theme: Theme) => string,
    private readonly renderError: (error: unknown, theme: Theme) => string,
    private readonly failureCooldownMs: number = DEFAULT_FAILURE_COOLDOWN_MS,
  ) {}
  ...
  private async refreshAsync(ctx: ExtensionContext): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        if (!this.active) return // session gone / model switched away mid-fetch
        this.lastData = data
  ...
  private apply(ctx: ExtensionContext, text: string): void {
    if (!this.active) return
    this.setStatusSafe(ctx, text)
  }

  /** setStatus that tolerates a ctx invalidated by session teardown. */
  private setStatusSafe(ctx: ExtensionContext, text: string | undefined): void {
    try {
